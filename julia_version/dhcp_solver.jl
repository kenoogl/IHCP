# -*- coding: utf-8 -*-
"""
直接熱伝導問題ソルバー（DHCP: Direct Heat Conduction Problem）
Python版からの変換: 係数行列構築と線形ソルバー
"""
...
"""
直接熱伝導問題の係数と右辺項の構築（並列化版）
Python の coeffs_and_rhs_building_DHCP 相当

引数:
- T_initial: 初期温度分布 (ni, nj, nk)
...
- b: 右辺項配列
"""
function coeffs_and_rhs_building_DHCP(T_initial::Array{Float64,3}, q_surface::Array{Float64,2},
...
- A_csr: CSR形式sparse行列
"""
function assemble_A_DHCP(ni::Int, nj::Int, nk::Int,
    ...
    # 対角線成分
    D0 = copy(a_p)                                    # 主対角線
    D_im1 = -a_s[(off_i+1):N]                        # i-1対角線
    D_jm1 = -a_w[(off_j+1):N]                        # j-1対角線
    D_km1 = -a_b[(off_k+1):N]                        # k-1対角線

    D_ip1 = -a_n[1:(N-off_i)]                        # i+1対角線
    D_jp1 = -a_e[1:(N-off_j)]                        # j+1対角線
    D_kp1 = -a_t[1:(N-off_k)]                        # k+1対角線

    # sparse行列構築
    A_csr = spdiagm(0 => D0,
                   -off_i => D_im1, -off_j => D_jm1, -off_k => D_km1,
                    off_i => D_ip1,  off_j => D_jp1,  off_k => D_kp1)
...
"""
直接問題の複数時間ステップソルバー
Python の multiple_time_step_solver_DHCP 相当

引数:
- T_initial: 初期温度分布 (ni, nj, nk)
- q_surface: 表面熱流束 (nt-1, ni, nj)
- nt: 時間ステップ数
- rho: 密度
...
戻り値:
- T_all: 全時間における温度分布 (nt, ni, nj, nk)
"""
function multiple_time_step_solver_DHCP(T_initial::Array{Float64,3}, q_surface::Array{Float64,3},
    ...
    ni, nj, nk = size(T_initial)
    T_all = Array{Float64}(undef, nt, ni, nj, nk)
    T_all[1, :, :, :] = T_initial

    # 初期解ベクトル（列優先順序でflat化）
    x0 = reshape(T_initial, :)
    ...
    for t in 2:nt
        ...
        # 結果を3D配列に再整形して保存
        T_all[t, :, :, :] = reshape(x_result, (ni, nj, nk))
